...
// Rate-limiting / IP blocking was intentionally disabled earlier (caused false positives).
const isLoginBlocked = () => false;
const getLoginBlockedMessage = () => '';
const recordFailedLoginAttempt = () => ({ failures: 0, lockedUntil: 0 });
...
const isLoginConfigured = () =>
  getAllowedLoginIds().length > 0 &&
  getConfiguredPassword().length > 0 &&
  getExpectedSessionToken().length > 0;
